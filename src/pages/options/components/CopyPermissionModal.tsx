    ...
    >
      <Space direction="vertical" className="w-full">
        ...
        <div className="mt-4">
          ...
          <div className="max-h-[200px] overflow-y-auto p-3 bg-gray-100 rounded-md text-xs">
            {permissionPoints.map((point, index) => (
              <div key={index} className="mb-1">
                <span className="text-gray-600">{point.authPointKey}:</span>
                <span className="ml-2">{point.authPointName}</span>
              </div>
            ))}
          </div>
        </div>
