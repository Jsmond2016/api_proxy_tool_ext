/**
 * 全局响应工具函数 - 使用 IndexedDB 存储全局响应配置
 */
 ...
 * 初始化 IndexedDB 数据库
 */
export const initGlobalResponseDB = (): Promise<IDBDatabase> => {
 ...
 * 保存全局 Mock 响应配置
 */
export const saveGlobalResponse = async (
  mockResponse: GlobalResponse
): Promise<void> => {
  const db = await initGlobalResponseDB()
 ...
 * 获取指定的全局 Mock 响应配置
 */
export const getGlobalResponse = async (
  id: string
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      resolve((request.result as GlobalResponse) || null)
    }
 ...
 * 获取所有全局 Mock 响应配置
 */
export const getAllGlobalResponses = async (): Promise<GlobalResponse[]> => {
  const db = await initGlobalResponseDB()
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const mocks: GlobalResponse[] = []

      request.onsuccess = (event) => {
        const cursor = (event.target as IDBRequest<IDBCursorWithValue>).result
        if (cursor) {
          mocks.push(cursor.value as GlobalResponse)
          cursor.continue()
        } else {
 ...
 * 删除全局 Mock 响应配置
 */
export const deleteGlobalResponse = async (id: string): Promise<void> => {
  const db = await initGlobalResponseDB()
 ...
 * 更新全局 Mock 响应配置
 */
export const updateGlobalResponse = async (
  mockResponse: GlobalResponse
): Promise<void> => {
  return saveGlobalResponse(mockResponse)
}

/**
 * 获取当前启用的全局 Mock 响应（单次只能开启一个）
 */
export const getActiveGlobalResponse =
  async (): Promise<GlobalResponse | null> => {
    const db = await initGlobalResponseDB()
    ...
    return new Promise((resolve) => {
      try {
        ...
        request.onsuccess = (event) => {
          const cursor = (event.target as IDBRequest<IDBCursorWithValue>).result
          if (cursor) {
            const mock = cursor.value as GlobalResponse
            if (mock.enabled) {
              // 找到第一个启用的记录
              resolve(mock)
              return
 ...
 */
export const updateGlobalResponseEnabled = async (
  id: string,
  enabled: boolean
): Promise<void> => {
  const db = await initGlobalResponseDB()
  ...
  return new Promise((resolve, reject) => {
    ...
    getRequest.onsuccess = () => {
      const mock = getRequest.result as GlobalResponse
      if (!mock) {
 ...
 * 确保单次只能开启一个（用于外部表格）
 */
export const toggleGlobalResponse = async (
  id: string,
  enabled: boolean
): Promise<void> => {
  const db = await initGlobalResponseDB()
  ...
  return new Promise((resolve, reject) => {
    ...
    if (enabled) {
      ...
      getRequest.onsuccess = () => {
        const mockToEnable = getRequest.result as GlobalResponse
        if (!mockToEnable) {
          reject(new Error("Global mock response not found"))
          return
        }
        ...
        const enabledMocks: GlobalResponse[] = []
