...
API Proxy Tool 用于管理浏览器中的 API Mock 规则。扩展通过 Manifest V3 `declarativeNetRequest` 将匹配的页面请求重定向到 Mock 地址，并提供 Apifox 同步、存档恢复和权限点复制等配套能力。
...
配置页从上到下分为：全局操作栏、模块标签页、接口 tag 信息栏、接口操作栏和接口表格。

| 区域            | 主要功能                                          |
| :-------------- | :------------------------------------------------ |
| 全局操作栏      | 全局开关、Apifox 配置、存档、全局权限点、全部重置 |
| 模块标签页      | “全部接口”聚合视图、模块切换、新建、重命名和删除  |
| 接口 tag 信息栏 | 展示当前模块接口涉及的 tag                        |
...
存档保存在浏览器 IndexedDB，内容是当前面板完整快照，包括全部模块、接口、快速联调配置和 Apifox 配置。tag 用于标识存档，不限制保存范围。

### 查看和恢复

在“存档”下拉菜单中选择“查看存档”，可查看详情、恢复或删除记录。恢复会覆盖当前模块、接口、Apifox 和快速联调配置，但保留当前全局 Mock 开关状态。
