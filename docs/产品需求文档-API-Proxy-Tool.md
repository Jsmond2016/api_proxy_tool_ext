...
支持从 Apifox 平台同步接口配置，自动将 Apifox 中的接口信息转换为产品可用的配置，支持基于标签（Tag）进行筛选，大幅减少手动配置的工作量。
...
- 显示 Apifox 文档中的所有标签
- **目录名过滤**：`x-apifox-folder` 目录名会混入 tag 列表，需过滤。提取完整值及按 `/` 分割的前缀路径，从可选 tag 中排除
- 支持多选，可以选择要同步的标签
...
- **groupName 处理**：未配置或格式无效的 groupName（如含中文「私教（PERSONAL-SAAS）」）的接口，统一归入 `demo.default` 分组，仅在 console 输出警告，不中断同步
- **校验失败提示**：选择标签后校验失败时，通过 `message.error` 弹出提示
- **数量上限**：单次 tag 筛选接口超过 60 个时，中断执行并弹出错误「筛选接口过多，请检查 tag 配置是否正确」
- **校验时机**：仅在选择 tag 后校验数量，弹框刚打开未选择 tag 时不校验
...
- 右侧：功能按钮组（全局开关、同步 Apifox、复制所有权限点、重置等）（注：导入导出已移除）
...
**Q: 如何备份配置？**
A: 导入导出功能已移除，建议通过 Apifox 同步或手动维护配置。

**Q: 如何在不同设备间同步配置？**
A: 导入导出功能已移除，建议使用 Apifox 同步方式在各设备上分别配置。
