...
从 Apifox 平台同步接口配置，支持基于 Tag 筛选，自动生成模块和 API 配置。
...
- **目录名过滤**：Apifox 的 `x-apifox-folder` 目录名会混入 tag 列表，需过滤。提取 `x-apifox-folder` 的完整值及其按 `/` 分割后的所有前缀路径，从可选 tag 中排除
- **Tag 历史**：保存最近选择的 Tag 组合，支持快速选择
- **Tag 冲突检测**：检测已存在的 Tag，提示用户选择合并策略
- **数量上限**：单次 tag 筛选接口数量上限为 60 个，超过时中断并提示「筛选接口过多，请检查 tag 配置是否正确」
- **校验时机**：仅在选择 tag 后校验数量，弹框刚打开未选择 tag 时不校验
...
- 根据 `x-apifox-fe-general-model-base-action-type` 字段分组
- 如果没有该字段，归入 `demo.default` 分组
- 未配置或格式无效的 groupName（如含中文）：统一归入 `demo.default` 分组，仅在 console 输出警告，不中断同步
- 模块 `apiDocUrl` 设置为 Apifox URL
- 模块 `apiDocKey` 从分组字段提取

**校验与提示**

- 选择标签后校验失败时（如 groupName 格式问题），通过 `message.error` 弹出提示
- 单次 tag 筛选接口超过 60 个时，中断执行并弹出错误提示
...
> **说明**：导入导出功能已移除，不再支持配置的 JSON 导入导出。
...
- **顶部导航栏**（64px 高度）
  ...
  - 右侧：全局开关、同步 Apifox、复制所有权限点、重置等按钮（注：导入导出功能已移除）
- **模块标签页区域**
  - 显示所有模块标签
  - 支持添加、编辑、删除模块
- **模块信息栏**（可选显示）
...
- **同步 Apifox 弹窗**
  - Apifox URL 输入和验证
  - Tag 选择器（多选，已过滤 x-apifox-folder 目录名）
  - Mock 地址前缀配置
  - 接口预览表格
  - 冲突检测提示
