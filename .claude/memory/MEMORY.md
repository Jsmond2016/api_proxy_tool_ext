...
- 完整下线“设置迭代信息”：删除导航入口、设置弹窗、缓存读写、文档展示与迭代/CR 复制，并移除存档中的迭代文档采集、预览、复制和恢复。
- 保留 Apifox tag 与存档功能；不主动删除旧 `chrome.storage` 数据，历史 IndexedDB 存档中的额外 `iterationInfo` 字段由恢复流程忽略。
- 同步更新中英文 README、用户指南、文档首页、Apifox tag 存档说明及 `main` 分支绑定文档。
- 全量 Vitest 26/26、定向 ESLint、Chrome 构建和文档构建通过；全量 lint 仍有 24 个既有错误/1 个警告，TypeScript 仍有 `SyncApifoxModal.tsx:144` 的 1 个既有错误。

## 2026-08-06

- Options 接口测试结果弹框新增“去改 mock”按钮，复用表格接口信息中的 Apifox 页面地址；无有效地址时隐藏。新增链接解析单测，22/22 测试、定向 ESLint 和 Chrome 构建通过；全量 lint 的 24 个既有错误/1 个警告及 TypeScript 的 2 个既有错误仍待处理。
